...
<body>
			...
			<div class="enter-infor-car__data">
				...
				<form action="${pageContext.request.contextPath}/CheckInsertUser"
					...
					<button type="submit" class="button enter-infor-car__button"
						value="<%=request.getParameter("image")%>">Buy</button>
 ...
 --%>
					<%-- <a
						href="/ShowRoomCar/TakeInfor?userName=<%=request.getParameter("userName")%>&fullName=<%=request.getAttribute("fullName")%>&phone=<%=request.getAttribute("phone")%>&address=<%=request.getAttribute("address")%>&carId=<%=request.getParameter("carId")%>&currentQuantity=<%=request.getParameter("currentQuantity")%>&brand=<%=request.getParameter("brand")%>&model=<%=request.getParameter("model")%>&price=<%=request.getParameter("price")%>&imageUrl=<%=request.getParameter("image")%>"
