...
<body>
	...
	<header class="header" id="header">
		<nav class="nav container">
			<a href="/ShowRoomCar/LoadCarsServlet" class="nav__logo"> <i
	...
	<main class="main">
		...
		<section class="gallery section">
			<div class="gallery__container container grid">
				<video class="video__slide active"
					src="/ShowRoomCar/SuperCarWebsite-main/SuperCarWebsite-main/assets/video/vid_1.mp4"
			...
			<div class="featured__container container">
				...
				<div class="featured__content grid">
					...
					<article
						...
						<a href="#"><h3
								class="featured__subtitle"><%=car.getModel()%></h3></a> <img
	...
	<!-- <script>
	// Get a reference to the login message and all featured cards
	const loginMessage = document.getElementById('login-message');
	const featuredCards = document.querySelectorAll('.featured__card');

	// Add a click event listener to each featured card
	featuredCards.forEach((card) => {
	    card.addEventListener('click', () => {
	        // Check if the user is logged in (you would implement your login logic here)
	        const userIsLoggedIn = /* Implement your login logic here, e.g., by checking a user session or cookie */ false;

	        if (!userIsLoggedIn) {
	            // If the user is not logged in, display the login message
	            loginMessage.style.display = 'block';
	        } else {
	            // If the user is logged in, you can proceed with the action, e.g., opening a modal or navigating to a new page
	            // Add your code here to handle the action when the user is logged in
	            // For example, open a modal, redirect to a login page, etc.
	        }
	    });
	});
