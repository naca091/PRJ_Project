...
<body>
		...
		<div class="featured__container container">
			<div class="featured__content grid">
				<c:forEach var="cartItem" items="${cartItems}">
					<article
						class="featured__card mix ${cartItem.getBrand().toLowerCase()}">
						<form action="/ShowRoomCar/InforUser" method="post">
							<div class="shape shape__smaller"></div>
							<span name="userName" style="display: none;">${cartItem.getUserName()}</span>

							<h1 class="featured__title" name="Brand">${cartItem.getBrand()}</h1>
							<a href="/ShowRoomCar/InforCar?carId=${cartItem.id}">
								<h3 class="featured__subtitle" name="Model">${cartItem.getModel()}</h3>
							</a> <img
								src="/ShowRoomCar/SuperCarWebsite-main/SuperCarWebsite-main/assets/img/${cartItem.image}"
								alt="" class="featured__img">
							<h3 class="featured__price" name="Price">$${cartItem.price}</h3>
							<form class="quantity-form">
								<input type="hidden" name="carId" value="${cartItem.id}">
								<input type="hidden" name="currentQuantity"
									value="${cartItem.quantity}">
								<button type="button" class="quantity-btn minus"
									data-action="decrement">-</button>
								<span name="quantity" class="quantity-value">${cartItem.quantity}</span>
							...
							<a class="button featured__button buy-button"
								onclick="confirmPurchase(${cartItem.id}, getQuantity())"
								type="submit">Buy</a>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        const decrementBtns = document.querySelectorAll(".quantity-btn.minus");
        const incrementBtns = document.querySelectorAll(".quantity-btn.plus");
        ...
        function handleQuantityChange(index, change) {
            let currentQuantity = parseInt(quantitySpans[index].textContent);

            if (currentQuantity + change > 0) {
                currentQuantity += change;
                quantitySpans[index].textContent = currentQuantity;
            } else {
        ...
        function confirmPurchase(index) {
            var quantity = parseInt(quantitySpans[index].textContent);
            var confirmResult = confirm(`Bạn có muốn mua chiếc xe này không?`);

            if (confirmResult) {
                var carId = document.querySelectorAll("input[name='carId']")[index].value;
                var brand = document.querySelectorAll(".featured__title")[index].innerText;
                var model = document.querySelectorAll(".featured__subtitle")[index].innerText;
                var price = document.querySelectorAll(".featured__price")[index].innerText;
                var userName = document.querySelectorAll("span[name='userName']")[index].innerText;

                // Get the image source from the img tag
                var image = document.querySelectorAll(".featured__img")[index].getAttribute("src");

                // Append additional parameters to the URL
                window.location.href = "/ShowRoomCar/InforUser?carId=" + carId +
                    "&userName=" + userName +
        ...
        function confirmRemoveFromCart(index) {
            // Implement the logic for confirming the removal from the cart if needed.
            // You may redirect to a remove from cart endpoint or perform other actions.
        }
    });
</script>
